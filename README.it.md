# PILCO-TCLab — Controllo termico con Reinforcement Learning model-based
...
PILCO affronta il problema apprendendo un **modello probabilistico della dinamica** (un Gaussian Process) e usandolo per pianificare la politica in simulazione, riducendo drasticamente il numero di interazioni reali necessarie.
...
Queste caratteristiche lo rendono un banco di prova ideale per PILCO. In questo lavoro il TCLab è modellato tramite un **simulatore ODE**.
...
├── util/          # Utility numeriche
│   ├── minimize.m        # ottimizzatore BFGS / L-BFGS / CG autonomo
│   └── gSat.m            # saturazione del segnale di controllo
├── scenarios/           # Scenari di riferimento della toolbox originale
│                        # (pendolo, cart-pole, uniciclo, …)
...
└── docs/                # Tesi (PDF) e guida ai casi di studio
```
...
- **Numero di episodi:** in tutti i casi PILCO converge a una politica efficace in **20–40 episodi** totali, rollout casuali inclusi.
...
- **Nessun toolbox aggiuntivo.** L'ottimizzazione usa `util/minimize.m`, un'implementazione autonoma di BFGS / L-BFGS / CG inclusa nel repository: l'Optimization Toolbox non serve.
- Il core di pilco-matlab è **già incluso** in questo repository (`base/`, `gp/`, `control/`, `loss/`, `util/`): non c'è nulla da clonare a parte.

**Esecuzione**

Ogni script dei casi di studio aggiunge da solo le cartelle dei moduli al path di MATLAB, con percorsi relativi alla propria cartella: vanno quindi eseguiti dall'interno della rispettiva directory.

```matlab
% ── Caso 1 — setpoint fisso (training e valutazione in un unico script) ──
cd pilco_case1
case1_learn_eval        % → results/case1_policy_trained.mat, results/figures/

% ── Caso 2 — robustezza alla temperatura ambiente ──
cd pilco_case2
case2_learn             % training → results/policy/case2_policy_trained.mat
case2_eval              % valutazione su Tamb mai viste in training

% ── Caso 3 — inseguimento di setpoint variabile ──
cd pilco_case3
case3_learn             % training
case3_eval              % valutazione sulla scalinata di setpoint mai vista

% ── Confronto PILCO vs isteresi ──
cd pilco_vs_isteresi
compare_hysteresis_pilco   % esegue entrambi i controllori e calcola le metriche
```

I grafici si rigenerano con gli script `draw_*` presenti in ogni cartella di caso (`draw_case1.m`, `draw_case2.m`, `draw_case3_step.m` e le rispettive versioni `*_training`).

## Documentazione

- [`docs/Tesi_PILCO_RL_Alberto_Bortoletto.pdf`](docs/Tesi_PILCO_RL_Alberto_Bortoletto.pdf) — l'elaborato completo.
- [`docs/Guida_Casi.md`](docs/Guida_Casi.md) — guida dettagliata ai tre casi di studio: scelte progettuali, parametri, cosa dimostra ciascun caso e i limiti di PILCO.
